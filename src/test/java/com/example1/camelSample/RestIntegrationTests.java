...
class RestIntegrationTests {
        ...
        void F112リモートからの取得リクエスト() throws Exception {
                ...
                Exchange origin = consumer
                                .receiveNoWait("file://./test/?fileName=testfile-utf8.dat&noop=true&idempotent=false");
                producer.send("sftp://hoge1@sftpSrv1:22/data?password=fuga1&fileName=f112-utf-lf.dat", origin);
        ...
        @Test
        void F113リモートからの取得リクエスト_SFTPwithProxy() throws Exception {
                String baseurl = "http://localhost:" + port;
                Exchange origin = consumer
                                .receiveNoWait("file://./test/?fileName=testfile-utf8.dat&noop=true&idempotent=false");
                producer.send("sftp://hoge3@sftpSrv3:22/data?password=fuga3&fileName=f113-utf-lf.dat", origin);
                String url = baseurl + "/api/nodeF?srcPath=data&srcFileNameWithExt=f113-utf-lf.dat";
                String expectedStr = """
                                {"status":"OK","message":"Request is Completed. 1 File Transfered","tranferedFileList":[{"consumedFileName":"f113-utf-lf.dat","producedFileName":"./test/to/113/F113-UTF-LF.DAT","result":"OK"}]}""";

                this.client.get().uri(url).exchange().expectBody(String.class).isEqualTo(expectedStr);

                File output = new File("./test/to/113/F113-UTF-LF.DAT");
                File expected = new File("./test/testfile-utf8.dat");
                assertEquals(true, output.exists());
                assertEquals(FileUtils.readFileToString(output, "UTF-8"),
                                (FileUtils.readFileToString(expected, "UTF-8")));
