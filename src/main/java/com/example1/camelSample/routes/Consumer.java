...
public class Consumer extends EndpointRouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        .process(exchange ->{    //ラムダ関数（引数ー＞｛引数に対する関数定義｝。関数宣言と実行を同時に行う）
            ...
            String fileName=exchange.getIn().getHeader("CamelFileAbsolutePath").toString(); //exchange(配達物）の中にあるラベル（ヘッダ）に書かれているファイル名を取得
        ...
        .to("direct:CenterHub")//中央郵便局に配送する
        .end()
        ;    
    
        from("ftp://test@localhost/tmp/from/?password=hoge&passiveMode=true&doneFileName=${file:name.noext}.trg&recursive=true")
